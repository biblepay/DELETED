...
namespace BiblePayPool2018
{
    public class Home : USGDGui, IRequiresSessionState
    {
        ...
        public WebReply Admin()
        {
            ...
            return wr2;
            // Start the multiple choice testing system
        ...
        public structQuestion GetQuestion(string sTestId, int iQuestionID)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                q.Narr = clsStaticHelper.ExtractXML(sAnswer, "<NARR>", "</NARR>").ToString();
                q.Answer = clsStaticHelper.ExtractXML(sAnswer, "<ANS>", "</ANS>").ToString();
        ...
        public WebReply btnNextQuestion_Click()
        {
            ...
            string sMode = Sys.GetObjectValue("ActiveTest", "Mode");
            int iQN = (int)clsStaticHelper.GetDouble(Sys.GetObjectValue("ActiveTest", "QuestionNumber"));
            if (sMode == "TEST")
            {
        ...
        public WebReply RosettaARMReport()
        {
            ...
            string sql = "select tempteam.id,tempteam.rosettaid, tempteam.UserName,sum(isnull(temphostdetails1.RAC,0)) as RAC, sum(isnull(temphostdetails2.rac,0)) as ARMRac "
                ...
                + " having sum(temphostdetails2.rac) > 0 ";
            ...
            w.bShowRowHighlightedByUserName = true;
            w.bSupportCloaking = true;
            WebReply wr = w.GetWebList(sql, "Rosetta Unbanked Report", "Rosetta Unbanked Report", "", "tempHosts", this, false);
            return wr;
        }
        ...
        public WebReply RosettaLeaderboard()
        {

            string sql = "select * from rosettaMaster order by rac desc";
            ...
            w.bShowRowHighlightedByUserName = true;
            w.bSupportCloaking = true;
            WebReply wr = w.GetWebList(sql, "Rosetta Leaderboard View", "Rosetta Leaderboard View", "", "tempHosts", this, false);
            return wr;

        }

        public WebReply RosettaMachineLeaderboard()
        {
            string sql = "select temphosts.id,tempteam.username as Username,temphostdetails.rac as MachineRAC, temphostdetails.added as Added,"
            ...
            SQLDetective s = Sys.GetSectionSQL("Rosetta Machine Leaderboard View", "tempTeam", string.Empty);
            s.OrderBy = "";
            Weblist w = new Weblist(Sys);
            w.bShowRowHighlightedByUserName = true;
            w.bSupportCloaking = true;
            WebReply wr = w.GetWebList(sql, "Rosetta Machine Leaderboard View", "Machine Leaderboard View", "","tempTeam", this, false);
            return wr;
        }
        ...
        {
            string sql = "select [users].id,[users].username, miners.username as minername,miners.highhpsmain as hps,"
                + "miners.updated from Miners inner join users on users.id = miners.userid  where miners.Updated > getdate() - 7 and "
                +" miners.Updated < getdate() - 1 / 48.01 and highhps" + Sys.NetworkID + " > 0"
                + " and users.id='" + Sys.UserGuid.ToString() + "'";
        ...
        public WebReply ProposalFundingChart()
        {
            string sql = "select Name,absoluteYesCount,NoCt,MasternodeCount, (AbsoluteYesCount)/(MasterNodeCount + .01) as popularity from proposal where Network='" 
                + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
        ...
        public double ExpenseTypeToBudgetAmount(string sNetworkID, string sExpenseType)
        {
            double dBudget = Convert.ToDouble(clsStaticHelper.GetGenericInfo(Sys.NetworkID, "getgovernanceinfo", "nextbudget", "result").ToString());
        ...
        public WebReply ProposalFundingByExpenseType()
        {
            
            string sql = "Select ExpenseType, sum(amount) amount From proposal where Network = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                double dBudget = ExpenseTypeToBudgetAmount(Sys.NetworkID,sExpenseType);
            ...
            string sSan = USGDFramework.clsStaticHelper.GetConfig("SAN");
            string sTargetPath = sSan + sFileName;
            if (clsStaticHelper.LogLimiter() > 990)     DeleteOrphanedPngFiles(sSan);
        ...
        {
            string sql = "Select * from Users with (nolock) where id='" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "' and deleted=0";
            System.Data.DataTable dt = Sys._data.GetDataTable2(sql);
            if (dt.Rows.Count > 0)
            {
                Sys.SetObjectValue("Account Edit", "Theme", dt.Rows[0]["Theme"].ToString());
                ...
                Sys.SetObjectValue("Account Edit", "CPID", dt.Rows[0]["CPID"].ToString());
                Sys.SetObjectValue("Account Edit", "AdoptedOrphanID", dt.Rows[0]["AdoptedOrphanID"].ToString());
                Sys.SetObjectValue("Account Edit", "AdoptionDate", dt.Rows[0]["AdoptionDate"].ToString());
                Sys.SetObjectValue("Account Edit", "LetterDeadline", dt.Rows[0]["LetterDeadline"].ToString());
                double dWAV = clsStaticHelper.GetDouble(dt.Rows[0]["WithdrawalAddressValidated"]);
            ...
            Edit geBtnRecVerify = new Edit("Account Edit", Edit.GEType.Button, "btnRecAddressVerify", "Enable Automatic Payments", Sys);

            AccountEdit.AddControl(geBtnRecVerify);
            ...
            Edit geCPID = new Edit("Account Edit", "CPID", Sys);
            geCPID.CaptionText = "CPID:";
            geCPID.TextBoxStyle = "width:420px";
            AccountEdit.AddControl(geCPID);
            AccountEdit.AddBlank();
            // Theme
            AccountEdit.AddBlank();
            ...
            Edit geBtnJoinDahf = new Edit("Account Edit", Edit.GEType.Button, "btnDAHFJoin", "Join DAHF Group", Sys);
            AccountEdit.AddControl(geBtnJoinDahf);
            ...
            */
            ...
            // BiblePay Adoption System
            string sql1 = "Select * from Orphans where id = '" + Sys.GetObjectValue("Account Edit", "AdoptedOrphanID") + "'";
            string sOrphanName = Sys._data.GetScalarString2(sql1, "Name", false);
            Sys.SetObjectValue("Account Edit", "AdoptedOrphanName", sOrphanName);


            Edit geOrphanName = new Edit("Account Edit", "AdoptedOrphanName", Sys);
            geOrphanName.CaptionText = "Adopted Orphan Name:";
            geOrphanName.Type = Edit.GEType.Text;

            geOrphanName.TextBoxAttribute = "readonly";
            geOrphanName.TextBoxStyle = clsStaticHelper.msReadOnly;
            geOrphanName.TextBoxStyle = "width:420px";

            AccountEdit.AddControl(geOrphanName);

            Edit geBtnAbandon = new Edit("Account Edit", Edit.GEType.Button, "btnAbandon", "Abandon Adopted Child", Sys);
            AccountEdit.AddControl(geBtnAbandon);

            Edit geAdoptionDate = new Edit("Account Edit", "AdoptionDate", Sys);
            geAdoptionDate.CaptionText = "Adoption Date:";
            geAdoptionDate.Type = Edit.GEType.Text;
            geAdoptionDate.TextBoxAttribute = "readonly";
            geAdoptionDate.TextBoxStyle = clsStaticHelper.msReadOnly;
            AccountEdit.AddControl(geAdoptionDate);
            AccountEdit.AddBlank();

            Edit geLWD = new Edit("Account Edit", "LetterDeadline", Sys);
            geLWD.CaptionText = "Letter Writing Deadline:";
            geLWD.Type = Edit.GEType.Text;
            geLWD.TextBoxAttribute = "readonly";
            geLWD.TextBoxStyle = clsStaticHelper.msReadOnly;
            AccountEdit.AddControl(geLWD);
            Edit geBtnWrite = new Edit("Account Edit", Edit.GEType.Button, "btnWriteToChild", "Write to your Child", Sys);
            AccountEdit.AddControl(geBtnWrite);

            // Automatic Withdrawals
            Edit geAW = new Edit("Account Edit", "WithdrawalAddressValidated", Sys);

            geAW.CaptionText = "Automatic Withdrawals:";
            geAW.Type = Edit.GEType.Text;
            geAW.TextBoxAttribute = "readonly";
            geAW.TextBoxStyle = clsStaticHelper.msReadOnly;
            AccountEdit.AddControl(geAW);

            Edit geBtnRecOff = new Edit("Account Edit", Edit.GEType.Button,
               "btnRecOff", "Turn Off Automatic Payments", Sys);
            AccountEdit.AddControl(geBtnRecOff);
        ...
        public WebReply btnRecAddressVerify_Click()
        {
            ...
            if (bValid)
            {
                ...
                string sql = "Update Users set WithdrawalAddressValidated=1 where id = '" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
                clsStaticHelper.mPD.Exec2(sql);
            }
            WebReply wr2 = d.CreateDialog("Notify", "BBP Address Verification", sNarr, 650, 350);
        ...
        public WebReply btnRecOff_Click()
        {
            ...
            string sAddress = Sys.GetObjectValue("Account Edit", "DefaultWithdrawalAddress");
            string sql = "Update Users set WithdrawalAddressValidated=0 where id = '" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
            clsStaticHelper.mPD.Exec2(sql);
            WebReply wr2 = d.CreateDialog("Notify", "BBP Address Verification", sNarr, 650, 350);
            WebReply wrAE = AccountEdit();
            wr2.AddWebPackages(wrAE.Packages);
        ...
        public WebReply btnAccountSave_Click()
        {
            ...
            string sql = "Update Users set DelName=@DelName,CPID=@CPID,Address1=@Address1,Phone=@Phone,Address2=@Address2,City=@City,State=@State,Zip=@Zip,Country=@Country,Updated=getdate(), Cloak='"
                + iCloak.ToString() + "', Email=@Email, Theme=@Theme, WithdrawalAddress=@DefaultWithdrawalAddress where id='"
                 + clsStaticHelper.GuidOnly(Sys.UserGuid)  + "' and deleted=0";
            ...
            {
                string sql2 = "Update Users set password='[txtAltPass]' where id= '" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                    + "' and deleted=0";
        ...
        public WebReply Vote_Click(string sVote)
        {
            ...
            string sWhere =" where letterid = '" + clsStaticHelper.GuidOnly(sId) 
                + "' and (userid = '" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                 + "' OR ip='" + clsStaticHelper.PurifySQL(sIP, 100)
                 + "')";
        ...
        public WebReply Announce()
        {
            ...
            if (o != null && i1 > 40) return (WebReply)o;
            ...
            double dPaidBounties = clsStaticHelper.GetTotalLetterBountiesPaid(1);
            double dTotalBounties = clsStaticHelper.GetTotalLetterBountiesPaid(0);
            ...
            sql = "Select sum(Amount) a from TransactionLog (nolock) where Added > getdate()-(2/24.01) and TransactionType = 'MINING_CREDIT'  and networkid = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' and userid='" + Sys.UserGuid.ToString() + "'";
            double dUC1 = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "a",false), 2) / 2;
            sql = "Select sum(Amount) a from TransactionLog (nolock) where Added > getdate()-(1) and TransactionType = 'MINING_CREDIT'  and networkid = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' and userid='" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                + "'";
            double dUC24 = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "a",false), 2);
            sql = "Select sum(Amount) a from TransactionLog (nolock) where Added > getdate()-(7) and TransactionType = 'MINING_CREDIT'  and networkid = '" + 
                clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' and userid='" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                + "'";
            double dUC7d = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "a",false), 2);
            ...
            if (true)
            {
                string sVideo = clsStaticHelper.mPD.GetScalarString2("Select Value as Video from System (nolock) where SystemKey='Video'", "Video",false);
                string sOrphanNews = "<xdiv style='width:400;height:400'><iframe src='" + sVideo
                    + "' /></xdiv>";
                    //                    + "<img width='150' height='150' src='https://pool.biblepay.org/images/announce.png' /> </ xdiv>";
                AddLabel("Announce", "vidCompassion", "Orphan News:", Announce, sOrphanNews);
            }

            sql = "Update Users set LastLogin=getdate() where id = '" + Sys.UserGuid.ToString() +"'";
            Sys._data.Exec2(sql);
        ...
        public WebReply MyTweets()
        {
            ...
            string sTweets = "<div style='width:100%; height:450; overflow:auto;'><a class='twitter-timeline' href='https://twitter.com/BiblePay?ref_src=twsrc%5Etfw'>Tweets by BiblePay</a> <script async src='https://platform.twitter.com/widgets.js' charset='utf-8'></script></div>";
        ...
        public WebReply MyCPID()
        {
            ...
            string sql = "Select CPID from Users where users.id='" + Sys.UserGuid.ToString() + "'";
            string sCpid = clsStaticHelper.mPD.GetScalarString2(sql, "CPID", false);
            sql = "Select * from Superblocks where CPID='" + sCpid + "' and height = (Select max(height) from superblocks)";
            DataTable dtCPID = clsStaticHelper.mPD.GetDataTable2(sql);
            ...
            else
            {
                ...
                AddLabelCSS("CPID", "lbl6", "Task Weight:", secCpid, dtCPID.Rows[0]["Taskweight"].ToString(), sStyle,"");
                ...
                AddLabelCSS("CPID", "lbl9", "Machine Count:", secCpid, dtCPID.Rows[0]["MachineCount"].ToString(), sStyle,"");
                AddLabelCSS("CPID", "lbl10", "Processor Count:", secCpid, dtCPID.Rows[0]["TotalProcs"].ToString(),sStyle,"");
                double dMagnitude = clsStaticHelper.GetDouble(dtCPID.Rows[0]["Magnitude"]);
        ...
        public WebReply About()
        {
            ...
            AddLabel("About", "lblFeeAnonymous", "Anonymous User Pool Fees:", About, ((dFeeAnonymous + dFee) * 100).ToString() + "%");
            //  Calculate load 
            ...
            double dHPS = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "hps"), 2);
            double dUsers = clsStaticHelper.mPD.GetScalarDouble2(sql, "users",false);
            ...
            sql = "Select sum(Amount) a from TransactionLog where Added > getdate() - 1 and TransactionType = 'MINING_CREDIT'  and networkid = '" 
                + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "'";
            double dCoins24 = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "a",false), 2);
            sql = "Select count(distinct height) b from blocks where updated > getdate() - 1 and networkid = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "'";
            double dB24 = Math.Round(clsStaticHelper.mPD.GetScalarDouble2(sql, "b",false), 2);
            ...
            // Last block Found
            sql = "select max(height) c,max(updated) d from blocks where networkid='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "'";
            double dLBF = clsStaticHelper.mPD.GetScalarDouble2(sql, "c",false);
            string sUpd = clsStaticHelper.mPD.GetScalarString2(sql, "d",false);
            ...
            // Pool Efficiency
            sql = "select(86400 * 7) / count(distinct height) / 60 e from blocks where updated > getdate() - 7 and networkid = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID) + "'";
            double d7 = Math.Round(clsStaticHelper.mPD.GetScalarDoubleWithNoLog2(sql, "e"), 2);
            sql = "select(86400 * 1) / count(distinct height) / 60 f from blocks where updated > getdate() - 1 and networkid = '" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID) + "'";
            double d1 = Math.Round(clsStaticHelper.mPD.GetScalarDoubleWithNoLog2(sql, "f"), 2);
            ...
            double mn1 = clsStaticHelper.mPD.GetScalarDouble2(sql, "a",false);
        ...
        public WebReply WritePics()
        {
            ...
            string sql = "Select * from Picture where parentid='" + clsStaticHelper.GuidOnly(sLetterGuid)
                + "'";
            System.Data.DataTable dt = Sys._data.GetDataTable2(sql);
            string sRow = "";
            if (dt.Rows.Count > 0)
        ...
        {
            string sId = Sys.LastWebObject.guid.ToString();
            string sql = "Select * from Letters where id = '" + clsStaticHelper.GuidOnly(sId) + "'";
        ...
        public WebReply WriteToOrphan()
        {
            ...
            geRecipient.CaptionText = "Orphan Name: " + Sys.GetObjectValue("Write", "Name") 
                + "<br>If you have problems in chrome adding a picture, Click the Menu Ellipsis | Settings | Scroll Down to Advanced and open | Scroll to the System Menu Div | Disable Use hardware acceleration.";
            ...
            string sql = "Select URL from Orphans where orphanid = '" + sLastOrphanID + "'";
            string sURL = clsStaticHelper.mPD.GetScalarString2(sql, "url");
            geBio.URL = sURL;
        ...
        public WebReply btnWriteSave_Click()
        {
            ...
            string sql = "Update Letters set body='" + clsStaticHelper.PurifySQLLong(sBody,3999)
                                + "',name='" + clsStaticHelper.PurifySQL(sTo,100)
                                + "',userid='"                + clsStaticHelper.GuidOnly(Sys.UserGuid)
                                + "',username='"              + clsStaticHelper.PurifySQL(Sys.Username,100)
                                + "',added=getdate(),orphanid='" + clsStaticHelper.PurifySQL(sOrphanID,100)
                                                + "' where id = '" + clsStaticHelper.GuidOnly(sLetterGuid)
                                                + "'";
            Sys._data.Exec2(sql);

            string sql2 = "Select AdoptedOrphanId from Users where id='" + Sys.UserGuid.ToString() + "'";
            string sOrphanId = Sys._data.GetScalarString2(sql2, "AdoptedOrphanID", false);
            if (sOrphanId.Length > 1)
            {
                sql2 = "Select orphanid from orphans where id = '" + sOrphanId + "'";
                ...
                if (dCt > 0)
                {
                    sql = "Update Users set LetterDeadline=getdate()+31 where id = '" + Sys.UserGuid.ToString() + "'";
                    Sys._data.Exec2(sql);
                }
            }
            return OrphanLetters();
        ...
        public WebReply btnBuy_Click()
        {
            ...
            string sql = "Select * from Products where id = '" + clsStaticHelper.GuidOnly(sBuyGuid) + "'";
            DataTable dt = clsStaticHelper.mPD.GetDataTable2(sql);
            Dialog d = new Dialog(Sys);
            WebReply wr;
            ...
            sql = "Select * from Users where id='" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
            DataTable dtU = clsStaticHelper.mPD.GetDataTable2(sql);
            ...
            if (sAddressVerified != "1")
            {
                ...
                {
                    sql = "Update users set AddressVerified='1' where id = '" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
                    clsStaticHelper.mPD.Exec2(sql);
                }
            }
            ...
            sql = "Update Orders Set Userid='" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                 + "',ProductId='"     + clsStaticHelper.PurifySQL(sProductID,60)
                 + "',Title='" +clsStaticHelper.PurifySQL( sTitle,200)
                 + "',Added=getdate(),Amount='" + dBBP.ToString() + "' where id = '" + clsStaticHelper.GuidOnly(sOrderGuid) + "'";
            clsStaticHelper.mPD.Exec2(sql);
            ...
            sql = "Update Orders set MouseId = '" + clsStaticHelper.PurifySQL(sMouseId,50)
        ...
        public WebReply StoreList()
        {
            ...
            string sql = "Select * from Products where Pics is not null and networkid='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' order by Added";
            System.Data.DataTable dt = Sys._data.GetDataTable2(sql);
            if (dt.Rows.Count > 0)
            {
        ...
        public WebReply NewsPics()
        {
            ...
            string sql = "Select * from Picture where addedby = '" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                + "' and parenttype='News'";
            System.Data.DataTable dt = Sys._data.GetDataTable2(sql);
            string sRow = "";
            if (dt.Rows.Count > 0)
        ...
        public WebReply Unbanked()
        {
            Section Unbanked = new Section("Unbanked", 1, Sys, this);

            AddLabel("Unbanked", "lbl1", 
                "<p>", Unbanked, 
                "<b><font color=green>Biblepay Services for the Unbanked v1.3<p><p>Use this page to associate your Rosetta Cancer Research mobile device with a Biblepay Receiving address.<p>"
                +" <font color=red><small></b>WARNING:  Do not use this page to associate your home PC wallet with a Rosetta Account.  This is strictly reserved for unbanked ARM, Tablet, and Mobile Device CPIDs.  The CPID will be marked as Unbanked and will not have the full capabilities of a normal home CPID. </small></b> <p><br> ");

            Edit geDestination = new Edit("Unbanked", "Address", Sys);
            geDestination.CaptionText = "BBP Receiving Address:";
            geDestination.TextBoxStyle = "width:300px";
            Unbanked.AddControl(geDestination);
            Edit geEmail = new Edit("Unbanked", "Email", Sys);
            geEmail.CaptionText = "Rosetta E-Mail Address:";
            geEmail.Width = "width=140px";
            Unbanked.AddControl(geEmail);
            
            Edit gePassword = new Edit("Unbanked", Edit.GEType.Password, "Password", "Rosetta Password:", Sys);
            Unbanked.AddControl(gePassword);
            Edit geBtnAssociate = new Edit("Unbanked", Edit.GEType.Button, "btnAssociate", "Associate Rosetta Account", Sys);
            Unbanked.AddControl(geBtnAssociate);
            return Unbanked.Render(this, true);
        }
        ...
        public WebReply btnAssociate_Click()
        {
            ...
            if (sResponse == "")
            {
                ...
                oParams[3] = "true";
                oParams[4] = sAddress;
                Bitnet.Client.BitnetClient bc2 = clsStaticHelper.GetBitNet(Sys.NetworkID);
        ...
        public WebReply Faucet()
        {
            ...
            string sReq = "<br><ul><font color=orange>Requirements:<li>Your CPID must be a member of team Biblepay in Rosetta@Home or World Community Grid<li>You must have a CPID that has not been paid by the faucet previously<li>You must have more than 100 RAC in either Rosetta@Home or World Community Grid<li>You must not be in any biblepay superblock as a researcher"
        ...
        public WebReply btnFaucetSend_Click()
        {
            ...
            string sSql = "Select count(*) as ct From Faucet where network='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' and (address = '" + clsStaticHelper.PurifySQL(sAddress, 50)
                + "' or IP='" + sIP + "')";
            double lIpHitCount = clsStaticHelper.GetScalarDouble2(sSql, "ct");
            double dRAC = clsStaticHelper.RetrieveCPIDRAC(sCPID, "team1") + clsStaticHelper.RetrieveCPIDRAC(sCPID, "team2");

            string sql2 = "Select count(*) as ct from Faucet where network='" + Sys.NetworkID + "' and Cpid = '" + sCPID + "'";
            double dCPIDcount = clsStaticHelper.GetScalarDouble2(sql2, "ct");
            if (dCPIDcount > 0)
            {
            ...
            double dSupercount = clsStaticHelper.GetScalarDouble2(sql3, "ct");
            if (dSupercount > 0)
            {
            ...
            if (dRAC < 100)
            {
                wr = d.CreateDialog("Withdrawal_Error", "Withdrawal Error", "Sorry, CPID does not have enough RAC (Rac is currently reading at " + dRAC.ToString() + ").  Please ensure you have switched your team to Biblepay, and your RAC > 100 in either Rosetta@Home or World Community Grid.", 150, 150);
                return wr;
            }

            string sql22 = "select max(added) upd from RequestLog where added > getdate() - 2";
            string sLast2 = clsStaticHelper.mPD.GetScalarString2(sql22, "upd");
            ...
            sql22 = "Select sum(amount) amt from RequestLog where userguid='" + clsStaticHelper.GuidOnly(Sys.UserGuid.ToString()) + "' and Notes='FAUCET'";
            double dAmt = clsStaticHelper.mPD.GetScalarDouble2(sql22, "amt");
            if (dAmt > 5 && false)
            {
            ...
            sql22 = "Select EmailVerified from Users where id='" + clsStaticHelper.GuidOnly(Sys.UserGuid.ToString()) + "'";
            double dV = clsStaticHelper.mPD.GetScalarDouble2(sql22, "EmailVerified");
            if (dV == 0 && false)
            {
            ...
            try
            {
                ...
                string sql4 = "Insert into RequestLog (username,userguid,address,id,txid,amount,added,network,ip,notes) values ('"
                    + clsStaticHelper.PurifySQL(Sys.Username, 100)
                    + "','" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                    + "','"
                    + clsStaticHelper.PurifySQL(sAddress, 120)
                    + "','" + sReqLogId + "','"
                    + "','" + (csAmt).ToString() + "',getdate(),'" + clsStaticHelper.VerifyNetworkID(this.Sys.NetworkID)
                    + "','" + clsStaticHelper.PurifySQL(sIP, 34)
                    + "','FAUCET')";
            ...
            string sql = "Insert into Faucet (id,height,transactionid,IP,amount,added,network,address,cpid) values (newid(),'"
                + iHeight.ToString() + "','"
                + sTXID.Trim() + "','"
                + sIP + "','" + csAmt.ToString() + "',getdate(),'" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID) + "','"
                + clsStaticHelper.PurifySQL(sAddress, 100)
                + "','" + clsStaticHelper.PurifySQL(sCPID, 100) + "')";
            Sys._data.Exec2(sql);
            //Send email to Jaap:
            string sBody = "Dear Airdrop, <br>BiblePay has transmitted " + csAmt.ToString() + " BBP to CPID " + sCPID + ".  <br><br>Thank you for using BiblePay!";
        ...
        public WebReply Tip()
        {
            ...
            string sql = "Select ID from Users where username='" + sUserId + "'";

            string sToGuid = clsStaticHelper.mPD.GetScalarString2(sql, "ID", false);
        ...
        public WebReply btnTipSend_Click()
        {
            ...
            double dAmount = clsStaticHelper.GetDouble(Sys.GetObjectValue("Tip", "Amount"));
            ...
            double dBalance = clsStaticHelper.AwardBounty(sGuid, sPayer, -1 * dAmount, "Tip", 0,
                                          sGuid, "main", sGuid, false);
            // Transfer the money to the Clicker
             clsStaticHelper.AwardBounty(sGuid, sToGuid, dAmount, "Tip", 0, sGuid, "main", sGuid, false);
        ...
        {
            string sql = "Select * from Users with (nolock) where id='" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                + "' and deleted=0";
            System.Data.DataTable dt = Sys._data.GetDataTable2(sql);
            if (dt.Rows.Count > 0)
            {
            ...
            geAvailableBalance.TextBoxStyle = clsStaticHelper.msReadOnly;
            ...
            geImmature.TextBoxStyle = clsStaticHelper.msReadOnly;
        ...
        {
            // Send funds
            string sql = "Select * From Users where id = '" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
            ...
            try
            {
                ...
                string sql2 = "Insert into RequestLog (username,userguid,address,id,txid,amount,added,network,ip) values ('"
                    + clsStaticHelper.PurifySQL(sName, 100)
                    + "','" + clsStaticHelper.GuidOnly(sGuid)
                    + "','"
                    + clsStaticHelper.PurifySQL( sAddress,120)
                    + "','" + sRLGuid + "','"
                    + "','" + (csAmt).ToString() + "',getdate(),'" + clsStaticHelper.VerifyNetworkID(this.Sys.NetworkID)
        ...
        public WebReply TestList_RowClick()
        {
            ...
            // Intro screen;
            string sql = "Select * from Test where ID='" + clsStaticHelper.GuidOnly(sId) + "'";
        ...
        {
            string sTestId = Sys.GetObjectValue("ActiveTest", "Id");
            double dQID = clsStaticHelper.GetDouble(Sys.GetObjectValue("ActiveTest", "QuestionNumber"));
            Sys.SetObjectValue("ActiveTest", "QuestionNumber", "1");
        ...
        {
            string sID = Sys.LastWebObject.guid.ToString();
            string sql = "Delete from Miners where ID='" + clsStaticHelper.GuidOnly(sID)
                + "'";
            try
            {
        ...
        {
            string sId = Sys.LastWebObject.guid.ToString();
            string sql = "Select URL,Name from Orphans where id = '" + clsStaticHelper.GuidOnly( sId)
                + "'";
            string sURL = Sys._data.GetScalarString2(sql, "URL");
            Section s = new Section("l1", 1, Sys, this);
            Dialog d = new Dialog(Sys);
            string sCSS = clsStaticHelper.msReadOnly;
        ...
        public WebReply AdoptionList()
        {
            ...
            if (ParentID == "") ParentID = Sys.Organization.ToString();
            s.WhereClause = " Id Not In (Select AdoptedOrphanId from Users where AdoptedOrphanId is not null) ";
            s.OrderBy = " updated";
            string sql = s.GenerateQuery();
            Weblist w = new Weblist(Sys);
            w.URLDefaultValue = "View Biography";
        ...
        public WebReply WriteLetter()
        {
            ...
            s.WhereClause += " and Id Not In (Select AdoptedOrphanId from Users where AdoptedOrphanId is not null)";
            s.OrderBy = " updated";
            string sql = s.GenerateQuery();
            Weblist w = new Weblist(Sys);
        ...
        public WebReply AdoptionList_ContextMenu_Adopt()
        {
            ...
            string sCSS = clsStaticHelper.msReadOnly;
            string sName = Sys._data.GetScalarString2(sql, "Name");
            sql = "Select AdoptedOrphanID from Users where id='" + Sys.UserGuid.ToString() + "'";
        ...
        public WebReply AdoptYes_Click()
        {
            ...
            string sql = "Update users set adoptedorphanid = '" + sID + "',letterdeadline=getdate()+31,adoptiondate=getdate() where id='" + Sys.UserGuid.ToString() + "'";
            Sys._data.Exec2(sql);
        ...
        {
            Dialog e = new Dialog(Sys);
            string sql = "Update Users Set AdoptedOrphanId = null,adoptiondate=null,letterdeadline=null where id = '" + Sys.UserGuid.ToString() + "'";
            Sys._data.Exec2(sql);
        ...
        public WebReply OrphanList_ContextMenu_Write()
        {
            ...
            string sql = "select count(*) ct from letters where added > getdate()-30 and body is not null and approved=0 and userid='" + clsStaticHelper.GuidOnly(Sys.UserGuid) + "'";
            double ct = clsStaticHelper.GetScalarDouble2(sql, "ct");
            if (ct >= 7)
            {
                Dialog d = new Dialog(Sys);
            ...
            sql = "Select OrphanID,Name from Orphans where id = '" + clsStaticHelper.GuidOnly(sOrphanID) + "'";
            Sys.SetObjectValue("Write", "Body", "");
            string sLastOrphanID = Sys._data.GetScalarString2(sql, "OrphanID");
            string sOrphanName = Sys._data.GetScalarString2(sql, "Name");
            // New Letter Guid
            string sLetterGuid = Guid.NewGuid().ToString();
            Sys.SetObjectValue("Write", "LetterGuid", sLetterGuid);
            string sql1 = "Insert into Letters (id) values ('" + clsStaticHelper.GuidOnly(sLetterGuid)
                + "')";
        ...
        {
            string sOrphanID = Sys.LastWebObject.guid.ToString();
            string sql = "Select ID,Name,Username,OrphanID,Body from Letters where id = '" + clsStaticHelper.GuidOnly(sOrphanID) + "'";
            string sLastOrphanID = Sys._data.GetScalarString2(sql, "OrphanID");
        ...
        string GetChartOfDifficulty(System.Drawing.Color color, string sChartName, string sFieldName)
        {
            string sql = "select Height,Difficulty,POWDifficulty,PODCDifficulty From Difficulty Where network = '"
                    + Sys.NetworkID + "' and height > 33260 and added > getdate()-30 order by Height";
            Chart c = new Chart();
            c.Width = 1500;
            ...
            s.BackSecondaryColor = GetBGColor();
            ...
            c.Legends[sChartName].BackColor = GetBGColor();
            c.Legends[sChartName].ForeColor = System.Drawing.Color.White;
        ...
        public WebReply DifficultyHistory()
        {
            ...
            string sFileName = GetChartOfDifficulty(System.Drawing.Color.Beige, "POW Difficulty", "powdifficulty");
            gePFL.URL = USGDFramework.clsStaticHelper.GetConfig("WebSite") + "SAN/" + sFileName;
            s1.AddControl(gePFL);
            Edit gePFLPODC = new Edit("Difficulty History", Edit.GEType.Image, "W2", "", Sys);
            string sFileName2 = GetChartOfDifficulty(System.Drawing.Color.Green, "PODC Difficulty", "podcdifficulty");
            gePFLPODC.URL = USGDFramework.clsStaticHelper.GetConfig("WebSite") + "SAN/" + sFileName2;
            s1.AddControl(gePFLPODC);
            return s1.Render(this, false);
        }
        ...
        public WebReply CPIDHistory()
        {
            ...
            sql = "select Height,Magnitude,UTXOWeight,TaskWeight From Superblocks where cpid='" + sCpid 
                + "' and height > 43000 order by Height";
            Chart c = new Chart();
            c.Width = 1500;
            ...
            c.Legends["Magnitude"].BackColor = GetBGColor();
            ...
            c.Legends["Task Weight"].DockedToChartArea = "ChartArea";
            c.Legends["Task Weight"].ForeColor = System.Drawing.Color.Yellow;
            ...
            Series sTask = new Series("TaskWeight");
            c.Series.Add(sTask);
            sTask.Color = System.Drawing.Color.Yellow;
            sTask.ChartType = SeriesChartType.FastLine;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                double Height = GetDouble(dt.Rows[i]["height"]);
                double dMagnitude = GetDouble(dt.Rows[i]["magnitude"]);
                double dUTXOWeight = GetDouble(dt.Rows[i]["utxoweight"]) / 10;
        ...
        public string GetFieldFromObjectBasedOnLastClick(string sFieldName, string sObjName)
        {
            ...
            string sql = "Select " + clsStaticHelper.PurifySQL(sFieldName,1000)
                + " From " + clsStaticHelper.PurifySQL(sObjName,50)
        ...
        public void InductMissingProposals(string sNetworkID)
        {
            ...
            oParams[1] = "all";
            oParams[2] = "proposals";
            dynamic o = clsStaticHelper.GetGenericObject(Sys.NetworkID, "gobject", oParams);
            ...
            foreach (var oIT in oResult)
            {
                ...
                if (tsDelta.Days < 60)
                {
                    ...
                    if (true)
                    {
                        ...
                        string sql = "insert into proposal (id,name,receiveaddress,amount,url,unixstarttime,unixendtime,preparetxid,"
                            ...
                            + "(newid(), '" + sName + "','" + sAddress + "','" + sAmount + "','" + sURL + "','" + sUnixStartTime + "','" + end_epoch + "','" + sCollateralHash + "'"
                            + ",getdate(),getdate(),null,'" + sCollateralHash + "','" + sNetworkID + "','1','1',null,'biblepay_core','" + sCreated + "','" + sCreated + "','"
                            + sProposalHash + "','" + sExpenseType + "')";
                        string sql2 = "Select count(*) ct from proposal where gobjectid = '" + sProposalHash + "'";
                        double dCount = clsStaticHelper.GetScalarDouble2(sql2, "ct");
                        if (dCount == 0)
                        {
                            clsStaticHelper.mPD.Exec2(sql);
                        }
        ...
        public void ProcessProposals(string sNetworkID)
        {
            ...
            string sql = "Select * from Proposal where paidtime is null and network='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID) + "'";
            DataTable dt = Sys._data.GetDataTable2(sql);
            for (int ii = 0; ii < dt.Rows.Count; ii++)
            {
                ...
                if (sPrepareTxId == "" && sHex != "")
                {
                    ...
                    string sql4 = "Update Proposal Set PrepareTxId='" + 
                        sPrepareTxid + "',SubmitTime=null,Submitted=0,gobjectid=null,SubmitTxId=null,PrepareTime=getdate() where id = '" + clsStaticHelper.GuidOnly(sId)
                        + "'";
                    Sys._data.Exec2(sql4);
                    clsStaticHelper.GetWL(Sys.NetworkID);
                }
                ...
                if (sTriggerTxId.Length > 10 && dAbsoluteYesCount > 0 && sPaidTime == "" && dHeight > 0)
                {
                    ...
                    if (sRawTx.Contains(sRecAddr) && sRawTx.Contains(dAmt.ToString()))
                    {
                        ...
                        sql = "update proposal set PaidTime=getdate(),SuperBlockTxId='" + sSuperBlockTxId + "' where id = '" + clsStaticHelper.GuidOnly(sId)
                            + "'";
                        Sys._data.Exec2(sql);
                    }
                    else
                    {
                        ...
                        {
                            sql = "update proposal set triggertxid=null,height=null,triggertime=null where id = '" + clsStaticHelper.GuidOnly(sId) + "'";
                            Sys._data.Exec2(sql);
                        }
                    }
                }
                ...
                if (sTriggerTxId == "" && bSupermajority && dBudgetable==1)
                {
                    ...
                    sql = "Select * from Proposal where Network='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                    ...
                    if (dtSuperblock.Rows.Count > 0)
                    {
                        ...
                        clsStaticHelper.Log("proposal trigger " + sTest);
                        try
                        {
                            if (sTriggerHex.Length > 10)
                            {
                                ...
                                clsStaticHelper.GetWLA(Sys.NetworkID, 60);
                                ...
                                for (int i = 0; i < dtSuperblock.Rows.Count; i++)
                                {
                                    ...
                                    string sql3 = "Update proposal set Height='" 
                                        ...
                                        + clsStaticHelper.GuidOnly(sProposalGuid) + "'";
                ...
                if (gObjectID.Length > 10)
                {
                    ...
                    if (v.Success && v.AbstainCount >= 0)
                    {
                        ...
                        sql = "update proposal set absoluteYesCount='" + v.AbsoluteYesCount.ToString() + "',YesCount='" + v.YesCount.ToString() + "',NoCt='" 
                            ...
                            + "' where id = '" + clsStaticHelper.GuidOnly(dt.Rows[ii]["id"].ToString()) + "'";
                        Sys._data.Exec2(sql);
                    }
                }
        ...
        public WebReply ProposalList()
        {
            ...
            WebReply wrChart2 = ProposalFundingByExpenseType();
        ...
        public WebReply btnProposalSave_Click()
        {
            ...
            clsStaticHelper.GetUserBalances(Sys.NetworkID,Sys.UserGuid, ref dOldBalance, ref dImmature);
            ...
            string sql = "Insert Into Proposal (id,ExpenseType,UserId,UserName,URL,name,receiveaddress,amount,unixstarttime,unixendtime,preparetxid,added,updated,hex,network,prepared,preparetime) "
                + " values ('" + sID + "','" + sExpenseType + "','" + Sys.UserGuid.ToString() + "','" + Sys.Username + "','" + sURL + "','" 
            ...
            sql = "Update proposal set MasternodeCount='" + dMNCount.ToString() + "' where network='" + clsStaticHelper.VerifyNetworkID(Sys.NetworkID)
                + "' and paidtime is null";
            if (dMNCount > 1) clsStaticHelper.mPD.Exec2(sql);
            //1-3-2018
            bool bTrue = clsStaticHelper.AdjustUserBalance(Sys.NetworkID, Sys.UserGuid, -1 * 2500);
            int iHeight = clsStaticHelper.GetTipHeight(Sys.NetworkID);
            clsStaticHelper.InsTxLog(Sys.Username, Sys.UserGuid, Sys.NetworkID,
        ...
        public WebReply btnDAHFJoin_Click()
        {
            Dialog d = new Dialog(Sys);
            WebReply wr = d.CreateDialog("dlgerr", "Welcome to DAHF", "Thank you for joining DAHF!  You can now log in to dahf.biblepay.org.", 240, 200);
            string sql = "Update Users Set DAHF=1 where Id='" + Sys.UserGuid.ToString() + "'";
            Sys._data.Exec2(sql);
        ...
        public WebReply btnAbandon_Click()
        {
            ...
            string sql = "Select ID,Name from Orphans where id = '" +sChildID + "'";
            Section s = new Section("l1", 1, Sys, this);
            Dialog d = new Dialog(Sys);
            string sCSS = clsStaticHelper.msReadOnly;
            string sName = Sys._data.GetScalarString2(sql, "Name");
        ...
        public WebReply btnLinkSave_Click()
        {
            ...
            string sql = "Insert Into Links (id,FaithCategory,username,Notes,Clicks,OriginalURL,URL,Added,UserId,PaymentPerClick,Budget) values ('" +
                sID + "','" + sFaithCategory.ToString() + "','" + Sys.Username.ToString() + "','" + clsStaticHelper.PurifySQL(sNotes,335)
                + "',0,'" + clsStaticHelper.PurifySQL(sURL,455)
                                + "','" + 
                                clsStaticHelper.PurifySQL(sShortURL,400)
                                + "',getdate(),'" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                                                + "','" + dPPC.ToString() 
                                                + "','" + dBudget.ToString() + "')";
            try
            {
        ...
        public WebReply DAHFNarrative(string sNarrative)
        {
            Section Faith = new Section("DAHF", 1, Sys, this);
            AddLabelWithCaptionWidth("DAHF", "lblA", "<p>", Faith, "Biblepay - DAHF Center", 11);
        ...
        public WebReply DAHFLinkList()
        {
            // Make a Section for the Narrative first
            string sWhere = " 1=1";

            SQLDetective s = Sys.GetSectionSQL("DAHF Link List", "DAHFLinks", string.Empty);
            if (ParentID == "") ParentID = Sys.Organization.ToString();
            ...
            WebReply wrLinkNarr = DAHFNarrative("DAHF CENTER");

            WebReply wrLinkList = w.GetWebList(sql, "DAHF Link List", "DAHF Link List", "", "DAHFLinks", this, false);
        ...
        public WebReply btnWorkerSave_Click()
        {
            ...
            string sql = "Insert Into Miners (id,UserId,Username,Notes,updated,added) values (newid(),'" + clsStaticHelper.GuidOnly(Sys.UserGuid)
                 + "',@Username,"  + "'" + clsStaticHelper.PurifySQL(sNotes,500) + "',getdate(),getdate())";
        ...
        public WebReply FormLoad()
        {
            ...
            if (bDAHF)
            {
                return Sys.Redirect("DAHF Home", this);
            }
            else
            {
