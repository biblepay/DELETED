...
namespace BiblePayPool2018
{
    ...
    public class SystemObject
    {
        ...
        public WebReply Redirect(string sPage,object Caller)
        {
            ...
            string sql = "Select ID,Sections from PAGE where deleted=0 and organization = '" + USGDFramework.Shared.GuidOnly(Organization.ToString())
                + "' and name='" + sPage + "'";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < vSections.Length; i++)
                {
                      ...
                      sql = "Select * From Section where Name='" + USGDFramework.Shared.PurifySQL(sSectionName,100)
                        + "' and deleted=0";
        ...
        public void BindSection(string sSectionName, string sTable, string sID)
        {
                ...
                string s2 = string.Empty;
                string Fields = GetViewFieldsForSection(sSectionName, out s1, out s2);
                string sql = "Select * FROM " + sTable + " WHERE ID='" + USGDFramework.Shared.GuidOnly(sID)
        ...
        public string GetPictureURL(string sPictureGuid)
        {
            ...
            string sql = "Select ID,FullFileName,Extension from Picture where id = '" + USGDFramework.Shared.GuidOnly(sPictureGuid) + "' and deleted=0";
            DataTable dt = _data.GetDataTable2(sql);
            if (dt.Rows.Count > 0)
            {
        ...
        public bool SendEmail(string strTo, string strSubject, string strBody, bool blnHTML = true, bool bCCBiblepay = false)
        {
            ...
            catch (Exception ex)
            {
                if (false)
                {
                    USGDFramework.Shared.Log("SendEmail: We encountered a problem while sending the outbound e-mail to " + strTo.ToString() + ", port " + smtp.Port.ToString() + ", host " + smtp.Host + ", Email error: " + ex.Message);
                }
                isSend = false;
            }
            return isSend;
        ...
        public string PrepareStatement(string sSection,string sql)
        {
            ...
            for (x = 0; x <= Information.UBound(vSql); x++)
            {
                ...
                if (true)
                {
                    ...
                    value = USGDFramework.Shared.PurifySQL(value, value.Length);
                    value = "'" + value + "'";
        ...
        public string GetNewDepositAddress()
        {
            ...
            string sql = "Update Uz set ReceiveAddress='" + sAddress + "' where id='" + USGDFramework.Shared.GuidOnly(this.UserGuid)
                + "' and deleted=0";
            _data.Exec2(sql);
            return sAddress;
        }
        ...
        public List<LookupValue> GetListOfOrgUsers()
        {
            ...
            string sql = "Select ID,Username from Uz where deleted=0 and organization = '" + USGDFramework.Shared.GuidOnly(Organization.ToString())
        ...
        void UpdateRecordWithChangeAudit(string sql,string sTable, string sGuid,bool bIsNew)
        {
            ...
            string sSelect = "SELECT * FROM " + sTable + " WHERE ID = '" + USGDFramework.Shared.GuidOnly(sGuid) + "'";
        ...
        public string GetBreadcrumbTrailHTML()
        {
            ...
            {
                string sURL = " style='color:lime;cursor:pointer;' onclick=postdiv(this,'buttonevent','" + _breadcrumb[i].Class + "','" 
                    + _breadcrumb[i].Method + "','','');";
