...
namespace BiblePayPool2018
{
    ...
    public class Section
    {
        ...
        public WebReply Render(object caller, bool ClearScreen)
        {
            ...
            {
                sBorderStyle = "style='width:95%; border-top: solid 1px; border-bottom:solid 1px; border-left: solid 1px; border-right:solid 1px;'";
            }
            else
            {
            ...
            string sOut = "<div id='" + divname + "' " + sDivClass + " name='" + divname + "'><table " + sBorderStyle 
                + " class='" + sContextMenuCssClass 
                + "' " + sFrameStyle + " cellspacing=5 cellpadding=5 width='100%' >" 
    ...
    public class Edit
    {
        ...
        public Edit(string sSection, string ControlName, SystemObject sys, string sReadOnlyCaptionText)
        {
            Name = ControlName;
            Type = GEType.Text;
            Sys = sys;
            ...
            CaptionText = sReadOnlyCaptionText;
            Type = Edit.GEType.Text;
            TextBoxAttribute = "readonly";
            TextBoxStyle = msReadOnly;
        }
        ...
        public Edit(string sSection, GEType ControlType, string ControlName, string ControlCaptionText, SystemObject sys)
        {
            ...
            if (ControlType==GEType.Button || ControlType==GEType.UploadControl2)
            {
                Method = Name  + "_Click";
            }

             if (ControlType==GEType.OldUploadControl || ControlType==GEType.UploadControl2)
             {
                 Method = Name + "_Click";
             }
            Sys = sys;
        ...
        public WebReply Render(object caller)
        {
            ...
            else if (Type == GEType.Image)
            {
                string sOut = "<td colspan=2 align=center><img name='"
                  + Name + "' id='" + Name + "' src='" + URL + "' style='width:100%' />&nbsp;</td>";
          ...
          + "        sOut2 += s;    }); ";
            ...
            else if (Type == GEType.UploadControl2)
            {
                string myClass = caller.GetType().ToString();
                StackTrace stackTrace = new StackTrace();
                StackFrame[] stackFrames = stackTrace.GetFrames();
                ...
                string sMyArgs = "parentid=" + ParentGuid + "&id=" + Id.ToString() + "&parenttype=" + ParentType;
                
                string sLaterExpand = "postdiv(this,'expand','BiblePayPool2018.Home','WritePics','','');";
                //string sTimeout = "setTimeout(\"" + sLaterExpand + "\", 4000);";

                string sEvent = "postdiv($('#myFile2').get(0),\"buttonevent\",\"" + myClass + "\",\"" + sMyMethod + "\",\"\",'');";
                

                string sJava = "$(\"#btnUpload2\").click(function(evt) {  if (window.FormData == undefined) alert('form data undefined');  var fileUpload = $('#myFile2').get(0);      var files = fileUpload.files;"
                    + "               var data = new FormData();"
                    + "               for (var i = 0; i < files.length; i++)"
                    + "              {"
                    + "                  data.append(files[i].name, files[i]);                 }  ";
                sJava += " $.ajax({ "
                    +"       url: '" + "Uploader.ashx?" + sMyArgs + "',  type: 'POST',  data: data,   contentType: false,      processData: false,  "
                    + "     success: function(result) { " + sEvent + "alert(result);" + " },  "
                    + "   error: function(err) {        alert(err.statusText + '[2]')                }"
                    + "        });                evt.preventDefault();                      });";

                

                    
                string sMyFile = "<label class='roundbutton'>" + CaptionText + "<input type='file' class='inputfile' id='myFile2' multiple onchange='btnUpload2.click();' />" + "</label>";

                string html = "<td colspan=2>"
                    + "<p id='DivUploadControl'></p>" + sMyFile
                    + "<button style='visibility:hidden' ID='btnUpload2' Text='Upload Selected File' /> "
                    + "&nbsp;&nbsp;"
                    + "<div id=divupload></div></td>";
                return new WebReply(html, sJava, Section, false);

            }
            else if (Type == GEType.OldUploadControl)
            {
                string myClass = caller.GetType().ToString();
                StackTrace stackTrace = new StackTrace();           
                StackFrame[] stackFrames = stackTrace.GetFrames();  
                ...
                string html = "<td colspan=2><form enctype='multipart/form-data' method='POST' action='Uploader.ashx'>"
                    + "<p id='DivUploadControl'>FileUpload 1.1</p>"
                    + "<input type='hidden' name='MAX_FILE_SIZE' value='8000000' />"
                    + "<label class='fileContainer roundbutton'>" + CaptionText
                    + "<input class='inputfile' type='file' "
                    + "id='myFile' name='myFile' xmultiple size=5 onchange=\"USGDFileUploader(this.form,'Uploader.ashx?parentid="
                    + ParentGuid + "&id=" + Id.ToString()
            ...
            else if (Type == GEType.Button)
            {
                ...
                string sMyMethod = method.Name;
                sMyMethod = Method;
                string sJavascriptSuffix = IsInDialog ? "$(this).closest('.ui-dialog-content').dialog('close').remove();" : String.Empty;
                string sOut = "";
            ...
            else if (Type == GEType.Anchor)
            {
                ...
                string sOut = "<td colspan=1><a name='" + Name + "' id='" + Name + "' value='"
                     + "' href=# onclick=\"" + sJavascriptSuffix + "postdiv(this,'buttonevent','" + myClass 
                     + "','" + this.Name + "_Click', '','');return true;\" >" + CaptionText + "</a></td>";
                return new WebReply(sOut, "", Section, false);
